(() => {
  ...
  const enabled = () => !document.hidden && !document.body.classList.contains('browser-open');
  function resize() {
  ...
  function draw(time) {
    ...
    for (const p of particles) {
      ...
      context.shadowColor = p.color; context.shadowBlur = 0;
  ...
  document.addEventListener('pointermove', event => {
    ...
    if (!lastPoint || distance >= 3) {
      const count = Math.min(5, Math.max(1, Math.ceil(distance / 12)));
      for (let i = 0; i < count; i++) {
        ...
        particles.push({x: lastPoint ? lastPoint.x + (point.x - lastPoint.x) * fraction : point.x,
          ...
          size: .8 + Math.random() * 1.4, life: duration, duration,
  ...
  const hide = () => { clear(); document.body.classList.remove('cursor-active'); };
